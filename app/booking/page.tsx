...
export default async function Page({searchParams}:{searchParams:Promise<{service?:string}>}){
  const [{service=""},p,services]=await Promise.all([searchParams,db.contentItem.findUnique({where:{type_slug:{type:"page",slug:"booking"}}}),db.service.findMany({where:{status:"PUBLISHED",active:true},orderBy:{displayOrder:"asc"},select:{slug:true,name:true,priceAmount:true,priceMode:true,priceCurrency:true,priceVisible:true}})]);
